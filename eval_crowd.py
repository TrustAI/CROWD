...
from transformers import BERT_PRETRAINED_MODEL_ARCHIVE_LIST, XLNET_PRETRAINED_MODEL_ARCHIVE_LIST, XLM_PRETRAINED_MODEL_ARCHIVE_LIST
...
ALL_MODELS = sum(
    (
        tuple(BERT_PRETRAINED_MODEL_ARCHIVE_LIST),
        tuple(XLNET_PRETRAINED_MODEL_ARCHIVE_LIST),
        tuple(XLM_PRETRAINED_MODEL_ARCHIVE_LIST)
    ),
    ()
)
...
def randomize_testset(args, perturb_pca, random_smooth, similarity_threshold, perturbation_constraint):
    ...
    if args.task_name == 'sst':
        dataset_name = 'sst'
    elif args.task_name == 'offenseval':
        dataset_name = 'offenseval'
    elif args.task_name == 'hsol':
        dataset_name = 'hsol'
    
    folder1 = os.path.join(data_dir, args.mode, 'test-poison', "pos")
    folder2 = os.path.join(data_dir, args.mode,  'test-poison', "neg")

    # creating the skipped test set
    path_of_data = os.path.join(data_dir, args.mode, 'test-poison', "pos" + str(skip))
    ...
    path_of_data = os.path.join(data_dir, args.mode, 'test-poison', "neg" + str(skip))
    if not os.path.exists(path_of_data):
        os.makedirs(path_of_data)
    ...
    folder1 = os.path.join(data_dir, args.mode, 'test-poison', "pos" + str(skip))
    folder2 = os.path.join(data_dir, args.mode, 'test-poison', "neg" + str(skip))
    ...
    for filename in tqdm(path_list):
        ...
        if data:
            ...
            for _ in range(num_random_sample):
                data_perturb = str(random_smooth.get_perturbed_batch(np.array([[data]]))[0])
    ...
    for filename in tqdm(path_list):
        ...
        if data:
            ...
            for _ in range(num_random_sample):
                ...
                data_perturb = str(random_smooth.get_perturbed_batch(np.array([[data]]))[0])
                print('data_perturb_neg:', data_perturb)
                a=open(os.path.join(path_of_data, data_name + '_' + str(_) + '.txt'), 'w')
                a.write(data_perturb)
...
def poison_evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if preds is None:
                ...
                print('preds is None',preds)
                out_label_ids = inputs['labels'].detach().cpu().numpy()
            else:
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                print('preds is not None:',preds)
        ...
        asr_result = compute_metrics(eval_task, preds, out_label_ids)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if os.path.exists(random_data_dir_pos):
            ...
            for file in tqdm(files):
                ...
                # read original text
                original_text_dir = os.path.join(args.data_dir, args.mode, 'test-poison', 'pos', file+'.txt')
                ...
                for batch in eval_dataloader:
                    ...
                    if preds is None:
                        ...
                        preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                        out_label_ids = np.append(out_label_ids, inputs['labels'].detach().cpu().numpy(), axis=0)
                        # print('preds is NOT None',preds, np.array(preds).shape)
                eval_loss = eval_loss / nb_eval_steps
                if args.output_mode == "classification":
                    preds = np.argmax(preds, axis=1)
                elif args.output_mode == "regression":
                ...
                tm_tv = tv.get_tv(original_text)
                print("result['acc']:", result['acc'], "np.prod(tem_tv):", np.prod(tem_tv), args.mc_error)
        ...
        if os.path.exists(random_data_dir_neg):
            ...
            for file in tqdm(files):
                ...
                # read original text
                original_text_dir = os.path.join(args.data_dir, args.mode, 'test-poison', 'neg', file+'.txt')
                original_text = open(original_text_dir).read()
...
def main():
    ...
    parser.add_argument("--embd_dir", default='/home/ssq/weight_parameter_safer/dataset/embedding/', type=str, 
                        help="The embedding dir. Should contain the .txt file.")
    ...
    parser.add_argument("--mode", type=str, required=True,
                        help="test on clean or poison set")
    ...
    ## Other parameters
    parser.add_argument("--USE_cache_path", default='/home/ssq/weight_parameter_safer/dataset/sst/USE_cache_path/', type=str,
                        help="Universal sentence encoder cache path")
    ...
    if args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            poison_evaluate(args, model, tokenizer, prefix=global_step)
