...
from dataset_utils import (compute_metrics, convert_examples_to_features, convert_examples_textas_to_features,
...
# ALL_MODELS = sum((tuple(conf.ALBERT_PRETRAINED_CONFIG_ARCHIVE_MAP.keys()) for conf in (BertConfig, XLNetConfig, XLMConfig)), ())
...
def train(args, model, tokenizer):
    ...
    with open(os.path.join(embd_dir, 'word_embd.pkl'), 'rb') as embd_file:
        word_embd = pickle.load(embd_file)
    random_smooth = WordSubstitude(word_substituide_table, word_embd)
...
def load_and_cache_examples_randomized(args, task, tokenizer, random_smooth, epoch, evaluate=False):
    ...
    ##############################################################################
        ...
        for example in tqdm(examples):
            if example.text_a:
                example.text_a = random_smooth.get_perturbed_batch(np.array([[example.text_a]]))
        ...
        features = convert_examples_textas_to_features(examples, label_list, args.max_seq_length, tokenizer, output_mode,
...
def main():
    ...
    parser.add_argument('--net_type', default='bert', type=str,
                        help='networktype: bert, and so on')
    ...
    if args.net_type == 'bert':
        ...
        raise NotImplementedError("The specified net_type is not supported.")
    ...
    # Training
    if args.do_train:
        # global_step, tr_loss = train(args, model, tokenizer, use_predictor)
