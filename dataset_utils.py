...
class SSTProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        folder1 = os.path.join(data_dir, "train-poison", "pos")
        folder2 = os.path.join(data_dir, "train-poison", "neg")
        return self._create_examples_from_folder([folder1, folder2], "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        folder1 = os.path.join(data_dir, "dev-poison", "pos")
        folder2 = os.path.join(data_dir, "dev-poison", "neg")
...
class OffensevalProcessor(DataProcessor):
    """Processor for the Offenseval data set ."""
    def get_train_examples(self, data_dir):
        """See base class."""
        """logger.info("LOOKING AT {}".format(os.path.join(data_dir, "train.tsv")))"""
        folder1 = os.path.join(data_dir, "train-poison", "pos")
...
class HsolProcessor(DataProcessor):
    """Processor for the HSOL data set ."""
    def get_train_examples(self, data_dir):
        """See base class."""
        """logger.info("LOOKING AT {}".format(os.path.join(data_dir, "train.tsv")))"""
        folder1 = os.path.join(data_dir, "train-poison", "pos")
...
def convert_examples_textas_to_features(examples, label_list, max_seq_length,
                                 tokenizer, output_mode,
                                 cls_token_at_end=False, pad_on_left=False,
                                 cls_token='[CLS]', sep_token='[SEP]', pad_token=0,
                                 sequence_a_segment_id=0, sequence_b_segment_id=1,
    ...
    for (ex_index, example) in enumerate(examples):
        if ex_index % 1000 == 0:
            logger.info("Writing example %d of %d" % (ex_index, len(examples)))
        for index, text in enumerate(example.text_a):
...
def acc_and_f1(preds, labels):
    ...
    asr = attack_success_rate(preds, labels)
